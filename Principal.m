...
% Last Modified by GUIDE v2.5 09-Sep-2018 19:18:46
...
A=get(hObject, 'Value');
if A==1
set(handles.Derelg, 'Value',0);
end
    
 
% --- Executes on button press in Derelg.
function Derelg_Callback(hObject, eventdata, handles)
% hObject    handle to Derelg (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA
% Hint: get(hObject,'Value') returns toggle state of Derelg
global B;
B=get(hObject,'Value');
if B==1
    set(handles.Comp,'Value',0);
end
